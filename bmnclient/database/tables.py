...
class CoinListTable(AbstractTable, name="coins"):
    ...
    def deserialize(self, cursor: Cursor, coin: abstract.Coin) -> bool:
        result = next(
            self._deserialize(
                cursor,
                type(coin),
    ...
    def serialize(self, cursor: Cursor, coin: abstract.Coin) -> None:
        self._serialize(
            cursor,
            coin,
            {self.Column.NAME: coin.name})
...
class AddressListTable(AbstractTable, name="addresses"):
    ...
    def deserializeAll(self, cursor: Cursor, coin: abstract.Coin) -> bool:
        assert coin.rowId > 0
    ...
    def serialize(self, cursor: Cursor, address: abstract.Coin.Address) -> None:
...
class TxListTable(AbstractTable, name="transactions"):
    ...
    def deserializeAll(
            self,
            cursor: Cursor,
            address: abstract.Coin.Address) -> bool:
        assert address.coin.rowId > 0
        assert address.rowId > 0
...
class TxIoListTable(AbstractTable, name="transactions_io"):
    ...
    def deserializeAll(
            ...
            coin: abstract.Coin,
            tx_row_id: int,
            io_type: IoType) -> Tuple[bool, List[abstract.Coin.Tx.Io]]:
        assert coin.rowId > 0
        assert tx_row_id > 0
        io_list = []
