...
def serializable(
    function: property | None = None, **kwargs
) -> Callable[[property], property] | property:
...
class Serializable:
        ...
        if cls.__serialize_map is None:
            ...
            for name in dir(cls):
                ...
                if isinstance(v, property) and getattr(
                    v.fget, "__serializable", None
                ):
                    cls.__serialize_map[StringUtils.toSnakeCase(name)] = dict(
                        name=name, **getattr(v.fget, "__data", {})
                    )
    ...
    def serialize(self, flags: SerializeFlag) -> DeserializedDict:
        return {
            key: self.serializeProperty(
                flags, key, getattr(self, data["name"])
            )
    ...
    def serializeProperty(
        ...
        raise TypeError(
            "can't serialize value type '{}' for key '{}'".format(
                str(type(value)), key
            )
        )
    ...
    @classmethod
    def deserialize(
        cls, flags: DeserializeFlag, source_data: DeserializedDict, *cls_args
    ) -> Serializable | None:
        cls_kwargs = {
            cls.kwargKeyFromName(key): cls.deserializeProperty(
                flags, None, key, value, *cls_args
            )
    ...
    def deserializeUpdate(
        self, flags: DeserializeFlag, source_data: DeserializedDict
    ) -> bool:
        kwargs = {
            self.kwargKeyFromName(key): self.deserializeProperty(
                flags, self, key, value
            )
            for key, value in source_data.items()
        }
    ...
    def deserializeProperty(
        ...
        *cls_args,
        ...
        raise TypeError(
            "can't deserialize value type '{}' for key '{}'".format(
                str(type(value)), key
            )
        )
