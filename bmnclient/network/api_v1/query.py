...
class AbstractApiQuery(AbstractJsonQuery):
    ...
    def url(self) -> Optional[str]:
        ...
        return NetworkUtils.urlJoin(
            self._server_url, self._VERSION, *path_list
        )
    ...
    def _processResponse(self, response: Optional[dict, list]) -> None:
        try:
            ...
            ResponseParser()(response, self._processData, self._processError)
        except ParseError as e:
            self._logger.error("Invalid server response: %s.", str(e))

    def _processError(self, error: int, message: str) -> None:
        self._logger.error(
            "Server error: %s", Logger.errorString(error, message)
        )
...
class AbstractOffsetIteratorApiQuery(AbstractApiQuery):
    ...
    class InitialData:
        ...
        def __eq__(
            self, other: AbstractOffsetIteratorApiQuery.InitialData
        ) -> bool:
            return (
        ...
        def __hash__(self) -> int:
            return hash((self._first_offset, self._last_offset))
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(*args, name_key_tuple=name_key_tuple, **kwargs)
        self._mode = mode
        ...
        if _initial_data is None:
            self._initial_data = self.InitialData(
                first_offset=self._first_offset, last_offset=self._last_offset
            )
        else:
    ...
    def _processData(
        ...
        for coin in self._coin_list:
            ...
            if not parser(value, coin.name):
                self._logger.warning(
                    "Coin '%s' not found in server response.", coin.name
                )
                continue
...
class HdAddressIteratorApiQuery(AddressInfoApiQuery):
    def __init__(
        ...
        *,
        ...
        _current_address: Optional[Coin.Address] = None,
    ) -> None:
            ...
            if not self._hd_iterator.broken_mode:
                self._next_query = self.__class__(
                    ...
                    _hd_iterator=HdAddressIterator(
                        self._address.coin, broken_mode=True
                    ),
                )
                return
...
class AddressTxIteratorApiQuery(
    AbstractOffsetIteratorApiQuery, AddressInfoApiQuery
):
    ...
    def __init__(
        ...
        *,
        ...
        _initial_data: Optional[
            AbstractOffsetIteratorApiQuery.InitialData
        ] = None,
    ):
        super().__init__(
            address,
    ...
    def _processData(
        ...
        for tx in parser.txList:
            tx_d = self._address.coin.Tx.deserialize(
                DeserializeFlag.NORMAL_MODE, tx, self._address.coin
            )
            if tx_d is not None:
                tx_d.save()
        ...
        self._updateAddressHistoryOffsets(
            parser.firstOffset, parser.lastOffset
        )
    ...
    def _updateAddressHistoryOffsets(
        self, first_offset: str, last_offset: Optional[str]
    ) -> None:
...
class AddressUtxoIteratorApiQuery(
    ...
    def __init__(
        ...
        *,
        first_offset: Optional[str] = None,
        last_offset: Optional[str] = None,
        _utxo_list: Optional[List[Coin.Tx.Utxo]] = None,
    ) -> None:
        super().__init__(
            address,
            name_key_tuple=CoinUtils.addressToNameKeyTuple(address),
    ...
    def _processData(
        ...
        for utxo in parser.txList:
            ...
            utxo_d = self._address.coin.Tx.Utxo.deserialize(
                DeserializeFlag.NORMAL_MODE, utxo, self._address
            )
...
class CoinMempoolIteratorApiQuery(AbstractApiQuery):
    _ACTION = ("coins", lambda self: self._coin.name, "unconfirmed")
    _DEFAULT_METHOD = AbstractApiQuery.Method.POST
    ADDRESS_COUNT_PER_REQUEST: Final = 50  # TODO dynamic

    def __init__(
        self, coin: Coin, *, _address_list: List[Dict[str, Any]] = None
    ) -> None:
    ...
    def _createData(self) -> Tuple[str, Any]:
        ...
        data = {"address_list": current_list["list"]}
    ...
    def _processTx(self, tx: DeserializedDict) -> None:
        for tx_io in chain(tx["input_list"], tx["output_list"]):
            ...
            tx_d = self._coin.Tx.deserialize(
                DeserializeFlag.NORMAL_MODE, tx, self._coin
            )
            if tx_d is not None:
                tx_d.save()
    ...
    def _processData(
        ...
        if len(self._address_list) > 0:
            self._next_query = self.__class__(
                self._coin, _address_list=self._address_list
            )


class TxBroadcastApiQuery(AbstractApiQuery):
    _ACTION = ("coins", lambda self: self._mtx.coin.name, "tx")
    _DEFAULT_METHOD = AbstractApiQuery.Method.POST
    ...
    def _createData(self) -> Tuple[str, Any]:
        return "tx_broadcast", {"data": self._mtx.raw().hex()}
    ...
    def _processData(
        ...
        self._logger.info(
            "Transaction '%s' broadcasted successfully!", parser.txName
        )
        self._result = parser
