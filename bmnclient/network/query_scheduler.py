...
class NetworkQueryScheduler:
    ...
    _TIMER_LIST: Tuple[Tuple[str, int], ...] = (
        ("updateCurrentFiatCurrency", Timer.UPDATE_FIAT_CURRENCY_DELAY),
        ("updateSysinfo", Timer.UPDATE_SERVER_INFO_DELAY),
        ("updateNewReleasesInfo", Timer.UPDATE_NEW_RELEASES_DELAY),
        ("updateCoinsInfo", Timer.UPDATE_COINS_INFO_DELAY),
    )

    _COIN_TIMER_LIST: Tuple[Tuple[str, int], ...] = (
        ("updateCoinHdAddressList", Timer.UPDATE_COIN_HD_ADDRESS_LIST_DELAY),
        ("updateCoinMempool", Timer.UPDATE_COIN_MEMPOOL_DELAY),
    )

    def __init__(
        self, application: CoreApplication, manager: NetworkQueryManager
    ) -> None:
        ...
        self._pending_queue: Dict[str, List] = {"coin_address": []}
    ...
    def _createTimerList(self) -> None:
        ...
        for callback_name, delay in self._TIMER_LIST:
            ...
            self._timer_list[name] = NetworkQueryTimer(
                delay, getattr(self, callback_name)
            )

        for callback_name, delay in self._COIN_TIMER_LIST:
            for coin in self._application.coinList:
                name = self._createTimerName(
                    self.COINS_NAMESPACE, callback_name, coin.name
                )
                self._timer_list[name] = NetworkQueryTimer(
                    delay, getattr(self, callback_name), (coin,)
                )
    ...
    def __queryFinishedCallback(
        self, timer: NetworkQueryTimer, query: AbstractQuery
    ) -> None:
    ...
    def _putQuery(
        self, query: AbstractQuery, **kwargs
    ) -> NetworkQueryManager.PutStatus:
    ...
    def _createRepeatingQuery(
        ...
        status = self._putQuery(
            query, unique=True, high_priority=high_priority
        )
    ...
    def updateCurrentFiatCurrency(self) -> None:
        ...
        self._createRepeatingQuery(
            service, (self.GLOBAL_NAMESPACE, "updateCurrentFiatCurrency"), True
        )
    ...
    def __pendingUpdateCoinAddress(
        self, query: AddressTxIteratorApiQuery, address: Coin.Address
    ) -> None:
    ...
    def broadcastTx(
        ...
        finished_callback: Callable[[int, Coin.TxFactory.MutableTx], None],
    ) -> bool:
