...
class TestHd(TestCaseApplication):
    ...
    def _assertKeys(
        self, node: HdNode, excepted_public_key: str, excepted_private_key: str
    ) -> None:
        self.assertIsNotNone(node)
        self.assertEqual(
            excepted_public_key,
            node.toExtendedKey(Bitcoin.bip0032VersionPublicKey, private=False),
        )
        ...
        self.assertEqual(
            excepted_public_key,
            node2.toExtendedKey(
                Bitcoin.bip0032VersionPublicKey, private=False
            ),
        )
        ...
        self.assertEqual(
            ...
            node.toExtendedKey(Bitcoin.bip0032VersionPrivateKey, private=True),
        )
        ...
        for account in range(0, 10):
            for is_change in (False, True):
                for i in range(10):
                    ...
                    self.assertEqual(
                        i, coin.nextHdIndex(purpose, account, change)
                    )
                    address = coin.deriveHdAddress(
                        account=account, is_change=is_change
                    )
                    self.assertIsNotNone(address)
                    self.assertTrue(address.save())
                    self.assertEqual(
                        i + 1, coin.nextHdIndex(purpose, account, change)
                    )
    ...
    def test_hd_list(self) -> None:
        for hd_list in HD_ADDRESS_LIST:
            ...
            for address in it:
                ...
                if path in hd_list:
                    count += 1
                    self.assertEqual(hd_list[path][0], address.name)
                    self.assertEqual(
                        hd_list[path][1], address.publicKey.data.hex()
                    )
                    key = coin.Address.importKey(coin, path)
                    self.assertIsNotNone(key)
                    self.assertEqual(
                        hd_list[path][1], key.publicKey.data.hex()
                    )
