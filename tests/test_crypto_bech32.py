...
class TestBech32(TestCase):
    def test_encode(self) -> None:
        self.assertEqual(
            None,
            Bech32.encode("", 1, b"12"))
        self.assertEqual(
            None,
            Bech32.encode("", 1, b"1234"))
        self.assertEqual(
            None,
            Bech32.encode("", 1, b"1234"))
        self.assertEqual(
            "f1pxyerxdql9k5af", # noqa
            Bech32.encode("f", 1, b"1234"))
        self.assertEqual(
            "f1qxyerxdqz6k94r", # noqa
            Bech32.encode("f", 0, b"1234"), )
        self.assertEqual(
            "f1sxyerxdqhxk0c8", # noqa
    ...
    def test_decode(self) -> None:
        ...
        self.assertEqual(
            result_none,
            Bech32.decode("12gny8wv"))
        self.assertEqual(
            result_none,
            Bech32.decode("bc1"))
        self.assertEqual(
            result_none,
            Bech32.decode("bc1_BAD_CHAR"))
        self.assertEqual(
            ("f", 16, b"1234"),
            Bech32.decode("f1sxyerxdqhxk0c8")) # noqa
        self.assertEqual(
            ("a", 0, b""),
            Bech32.decode("a1q3g6mn3"))
        self.assertEqual(
            ("f", 0, b"1234567890" * 1),
            Bech32.decode("f1qxyerxdp4xcmnswfsj7r2mj")) # noqa

        for (address, hrp, version, value) in TEST_LIST:
            self.assertEqual(
                (hrp, version, bytes.fromhex(value)),
