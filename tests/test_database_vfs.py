...
class TestDatabaseVfs(TestCaseApplication):
    ...
    def _test_sector(self, sector_size: int):
        ...
        for i in range(sector_count):
            sector = i.to_bytes(1, "little") + b"".join(
        ...
        for i in range(1, sector_count):
            ...
            size = (
                file.sectorSize // 2 + file.sectorSize + file.sectorSize // 2
            )
            self.assertEqual(
                shadow[offset : offset + size], file.read(size, offset)
            )
